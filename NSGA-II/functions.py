...
def build_gene(r, DAYS):
    # Vehicle (allow overflow → penalized later)
    v_id = random.randint(1, 3)
...
def print_solution(individual, requests, DAYS):
    print("Name = Uzair Mustafa")
    ...
    for day in range(1, DAYS + 1):
        ...
        print(f"DAY = {day} NUMBER_OF_VEHICLES = {len(v_day)}")
        ...
        for v_id, req_list in v_day.items():
            ...
            print(f"(V_ID: {v_id})\t 1", *route_locs, "1")

            print(f"(REQ_IDS: \t", *route, ")")
        ...
        for s_id, req_list in s_day.items():
            ...
            print(f"(S_ID: {s_id})\t {home_loc}", *route_locs, home_loc)
    ...
    print(f"TOTAL_DELAY_PENALTIES = {int(metrics['penalties'])}")
